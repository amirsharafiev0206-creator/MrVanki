...
def save_history():
    try:
        ...
        logging.error("Ошибка сохраненния истории: %s", e)
...
def chat(user_id, text):
    try:
        if str(user_id) not in history:
            ...
            data = {
                "model": "deepseek-ai/DeepSeek-R1-0528","messages": history[str(user_id)]}
            ...
            if isinstance(data, dict) and data.get('choices'):
                ...
                logging.error((f"jib,rf API: {json.dumps(data,ensure_ascii=False)}"))
        except Exception as e:
            logging.error(f"ошибка при запросе: {e}, повторите")
            send_long_message(f"ошибка при запросе: {e}, повторите позже")
data = {"users": {}}
db_path = "db.json"
...
def dice_game(message):
    ...
    bot.send_message(message.chat.id, "Угадайте число на кубике")
...
def slot_game(message):
    ...
    if value in (16, 32, 48):
        ...
        bot.send_message(message.chat.id, "Ты проиграл")
            # message - объект, хранящий информацию о сообщении пользователя
    # message.text - текст сообщения
    # message.chat.id - id чата пользователя, отправившего сообщение
    # bot.send_message(id, content) - отправляет сообщение на указанный id с содержимым, указанным в content
if __name__ == "__main__":
    server_url = os.getenv("RENDER_EXTERMAL_URL")
    if server_url and TOKEN:
        webhook_url = f"{server_url.rsplit('/')}/{TOKEN}"
        try:
